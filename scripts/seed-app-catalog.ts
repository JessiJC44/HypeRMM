...
const CATALOG = [
  {
    ...
    installCommands: {
      windows: { source: 'custom_url', packageId: 'hyperemote-agent-windows.exe',
        customInstallScript: "Invoke-WebRequest '/api/agent/download/windows' -OutFile $env:TEMP\\agent.exe; Start-Process $env:TEMP\\agent.exe -Wait" },
      mac: { source: 'custom_url', packageId: 'hyperemote-agent-mac-arm',
        customInstallScript: "curl -sSL /api/agent/download/mac -o /tmp/agent && chmod +x /tmp/agent && /tmp/agent" },
      linux: { source: 'custom_url', packageId: 'hyperemote-agent-linux',
        customInstallScript: "curl -sSL /api/agent/download/linux -o /tmp/agent && chmod +x /tmp/agent && /tmp/agent" },
    },
    isBuiltIn: true,
  },
